...
- `xcodebuild -project MythConf/MythConf26.xcodeproj -scheme MythConf26 -destination 'platform=iOS Simulator,name=iPhone 17e' build` builds the app for the local simulator.
...
Use the local `iPhone 17e` simulator for build/run verification unless a task requires another device.

For SwiftUI color updates, prefer dynamic system colors that adapt to light mode, dark mode, and inverted colors. For the programme day picker, use `Color(.systemBlue)` for the rounded container, `.systemCyan` for `UISegmentedControl.appearance().selectedSegmentTintColor`, white text for normal segments, and black text for the selected segment.
